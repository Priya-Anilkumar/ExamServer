...
export class ChooseDrinkComponent implements OnInit {
  ...
  constructor(
    ...
    private ds: DrinkService,
    private route: ActivatedRoute,
    private sls: SaleListService
  ) {}
  ...
  addSaleList(dr: Drink) {
    this.numberOfCup = Number(
      (<HTMLInputElement>document.getElementById(`numberOfCup_${dr.id}`)).value
    );
    ...
    if (noc == 0) {
      ...
      this.cs.getAvailableContainerValue().subscribe((rs) => {
        ...
        this.newCtnValue.coffeeContainer =
          rs.coffeeContainer - (dr.coffee * noc) / 1000;
        this.newCtnValue.milkContainer =
          rs.milkContainer - (dr.milk * noc) / 1000;
        this.newCtnValue.sugarContainer =
          rs.sugarContainer - (dr.sugar * noc) / 1000;
        this.newCtnValue.waterContainer =
          rs.waterContainer - (dr.water * noc) / 1000;
        //check if containers are enough for making drinks
        if (
          this.newCtnValue.teaContainer <= 0 ||
          this.newCtnValue.coffeeContainer <= 0 ||
          this.newCtnValue.milkContainer <= 0 ||
          this.newCtnValue.sugarContainer <= 0 ||
          this.newCtnValue.waterContainer <= 0
        ) {
          this.alert_type = 'alert-warning';
          this.msg =
            'Containers are not enough. Please contact to ADMIN for refilling!';
        } else {
          // update containers with new avalue
          //this.cs.updateCurrentContainer(this.newCtnValue).subscribe(rs => {});
          // add sales into salelist
          this.sls
            .addSaleList(this.setSaleListValue(dr, noc))
            .subscribe((rs) => {
              if (rs.saleListId != 0) {
