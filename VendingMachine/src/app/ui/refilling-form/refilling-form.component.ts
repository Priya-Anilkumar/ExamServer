...
export class RefillingFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.authService.currentUser.subscribe((user) => {
      if (user == null) {
        ...
        this.cs.getMaxContainerValue().subscribe((c) => {
          this.maxCtn = c;
        });
        this.cs.getAvailableContainerValue().subscribe((c) => {
          this.availableCtn = c;
        });
      }
    });
  ...
  addRefill(refillForm) {
    ...
    if (
      refillForm.value.teaRefill > 0 ||
      refillForm.value.coffeeRefill > 0 ||
      refillForm.value.waterRefill > 0 ||
      refillForm.value.sugarRefill > 0 ||
      ...
      this.rs.addRefill(rf).subscribe((rs) => {
        this.msg =
          rs.refillId != 0
            ? 'Containers are refilled successfully!'
            : 'Errors!';
